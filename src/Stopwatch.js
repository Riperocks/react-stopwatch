...
function calculateTime() {
    ...
    let millisecondsTimer = getZeroAppendedString(String(millisecondsPassed % 1000).substring(0, 2));
     ...
     <div className="container">
        <div className="clock">
            ...
            <div className="button-container">
                <div onClick={startTimer} className={`button ${isStarted ? 'active' : ''}`}>{isStarted ? 'Stop' : 'Start'}</div>
                <div onClick={resetTimer} className="button">Reset</div>
            </div>
        </div>
    </div>
    );
